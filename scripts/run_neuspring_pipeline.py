...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        nargs="+",
        required=True,
        help="One or more target total spring ratios relative to Full.",
    )
...
def main() -> None:
    ...
    if any(not 0.0 < ratio <= 1.0 for ratio in a.target_final_ratios):
        raise ValueError("Every target final ratio must be in (0,1]")
    if any(ratio <= a.source_final_ratio for ratio in a.target_final_ratios):
        raise ValueError(
            "NeuSpring recovery restores springs, so every target ratio must "
            "be larger than --source-final-ratio"
        )
    ...
    for target_ratio in a.target_final_ratios:
    ...
    overall = {
        "protocol": "generic_neuspring_multi_budget_sweep",
        "scene": a.scene,
        "source_final_ratio": float(a.source_final_ratio),
        "target_final_ratios": list(map(float, a.target_final_ratios)),
